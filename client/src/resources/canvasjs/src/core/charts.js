...
Chart.prototype._updateOptions = function () {
  ...
  if (this._toolBar.style.display !== "none" && this._zoomButton) {
    ...
    if (
      _this._resetButton.getAttribute("state") !== _this._cultureInfo.resetText
    )
  ...
  for (var series = 0; series < this._options.data.length; series++) {
    ...
    if (
      !(
        ...
        Chart._supportedChartTypes.indexOf(this._options.data[series].type) >= 0
      )
    )
      continue;
...
Chart.prototype.render = function (options) {
  ...
  if (this.axisY && this.axisY2) {
    if (
      ...
      typeof this.axisY2._options.gridThickness === "undefined"
    )
      this.axisY2.gridThickness = 0;
    else if (
      this.axisY2.gridThickness > 0 &&
      typeof this.axisY._options.gridThickness === "undefined"
    )
      this.axisY.gridThickness = 0;
  }
  ...
  if (this._axes.length > 0 && (this.zoomEnabled || this.panEnabled)) {
    for (var i = 0; i < this._axes.length; i++) {
      if (
        this._axes[i].viewportMinimum !== null ||
        this._axes[i].viewportMaximum !== null
      ) {
        showToolBar = true;
        break;
      }
...
Chart.prototype._processMultiseriesPlotUnit = function (plotUnit) {
  ...
  for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
    ...
    if (
      ...
      var plotAreaXMin = this.sessionVariables.axisX.newViewportMinimum
        ? this.sessionVariables.axisX.newViewportMinimum
        : this._options.axisX && this._options.axisX.viewportMinimum
          ? this._options.axisX.viewportMinimum
          : this._options.axisX && this._options.axisX.minimum
            ? this._options.axisX.minimum
            : -Infinity;

      var plotAreaXMax = this.sessionVariables.axisX.newViewportMaximum
        ? this.sessionVariables.axisX.newViewportMaximum
        : this._options.axisX && this._options.axisX.viewportMaximum
  ...
  for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
    ...
    for (i = 0; i < dataSeries.dataPoints.length; i++) {
      ...
      if (i > 0) {
        ...
        if (
          dataPointY[0] !== null &&
          dataSeries.dataPoints[i - 1].y[0] !== null
        ) {
    ...
    for (var i = 0; i < this._events.length; i++) {
      ...
      if (
        ...
        if (type === "mousedown" && eventParam.capture === true) {
          ...
          if (this.overlaidCanvas.setCapture) this.overlaidCanvas.setCapture();
          else {
...
Chart.prototype._plotAreaMouseUp = function (x, y) {
  if (
    ...
    if (this.isDrag) {
      var dragDeltaPY = y - this.dragStartPoint.y,
        ...
        if (this.panEnabled) {
          ...
          for (var i = 0; i < this._axes.length; i++) {
            ...
            if (axis.viewportMinimum < axis.minimum) {
              ...
              axis.sessionVariables.newViewportMinimum =
                axis.viewportMinimum + overflow;
              axis.sessionVariables.newViewportMaximum =
                axis.viewportMaximum + overflow;
          ...
          (!zoomPX || Math.abs(dragDeltaPX) > 2) &&
          (!zoomPY || Math.abs(dragDeltaPY) > 2) &&
          this.zoomEnabled
        ) {
          ...
          if (
            Math.abs(selectedRegion.x1 - selectedRegion.x2) > 2 &&
            Math.abs(selectedRegion.y1 - selectedRegion.y2) > 2
          ) {
            if (
...
Chart.prototype._plotAreaMouseMove = function (x, y) {
  if (this.isDrag && this.plotInfo.axisPlacement !== "none") {
    ...
    if (
      Math.abs(dragDeltaPX) > 2 &&
      Math.abs(dragDeltaPX) < 8 &&
...
Chart.prototype._zoomPanToSelectedRegion = function (
  px1,
  py1,
  px2,
  py2,
  ...
  for (var i = 0; i < axes.length; i++) {
    ...
    if (isFinite(axis.dataInfo.minDiff)) {
      if (
        !(
          Math.abs(val2 - val1) < 3 * Math.abs(axis.dataInfo.minDiff) ||
          val1 < axis.minimum ||
          val2 > axis.maximum
        )
      ) {
...
Chart.prototype.renderIndexLabels = function (targetCtx) {
  ...
  for (var i = 0; i < this._indexLabels.length; i++) {
    ...
    if (
      ...
      if (
        indexLabel.dataPoint.x < axisX.viewportMinimum ||
        indexLabel.dataPoint.x > axisX.viewportMaximum ||
        indexLabel.dataPoint.y < axisY.viewportMinimum ||
        indexLabel.dataPoint.y > axisY.viewportMaximum
      )
        continue;
    } else {
      if (
...
Chart.prototype.renderLine = function (plotUnit) {
  ...
  for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
    ...
    ghostCtx.lineWidth =
      dataSeries.lineThickness > 0 ? Math.max(dataSeries.lineThickness, 4) : 0;
...
Chart.prototype.replaceKeywordsWithValue = function (
  ...
  if (
    ...
    do {
      ...
      if (ds.percentFormatString) percentFormatString = ds.percentFormatString;
      else {
