...
DataSeries.prototype.getDataPointAtX = function (x, findClosest) {
  ...
  else if (findClosest && searchResult.dataPoint !== null) return searchResult;
  else return null;
};
  ...
  while (true) {
    ...
    if (i >= 0 && i < this.dataPoints.length) {
      ...
      if (visualInfo) {
        switch (this.type) {
          ...
          case "rangeSplineArea":
            ...
            distance = Math.min(
              ...
              Math.sqrt(
                Math.pow(visualInfo.x1 - x, 2) + Math.pow(visualInfo.y2 - y, 2),
              ),
            );
            if (distance <= snapDistance) {
              results.push({
          ...
          case "doughnut":
            ...
            var innerRadius =
              this.type === "doughnut"
                ? visualInfo.percentInnerRadius * visualInfo.radius
                : 0;
        ...
        if (
          foundDataPoint ||
          (forwardMissCount > maxMissCount && backwardMissCount > maxMissCount)
        )
          break;
      }
    } else if (
