...
ToolTip.prototype._updateToolTip = function (mouseX, mouseY) {
  if (this.chart.disableToolTip) return;
  ...
  if (
    ...
    for (i = 0; i < nearbyEntries.length; i++) {
      if (
        nearbyEntries[i].dataPoint.x.valueOf() === closest.dataPoint.x.valueOf()
      )
    ...
    if (this.currentSeriesIndex >= 0) {
      ...
      if (this.currentDataPointIndex >= 0) {
        ...
        var x =
          dataSeries.axisX.conversionParameters.minimum +
          (mouseX - dataSeries.axisX.conversionParameters.reference) /
            dataSeries.axisX.conversionParameters.pixelPerUnit;
          ...
          if (entry.dataPoint.y.length > 0) {
            ...
            for (var i = 0; i < entry.dataPoint.y.length; i++)
              if (entry.dataPoint.y[i] < entry.dataSeries.axisY.viewportMinimum)
                ...
                entry.dataPoint.y[i] > entry.dataSeries.axisY.viewportMaximum
              )
                unboundToViewport++;
            if (
              unboundToViewport < entry.dataPoint.y.length &&
              unboundToViewport > -entry.dataPoint.y.length
            )
              entries.push(entry);
          } else {
            if (
              entry.dataPoint.y >= entry.dataSeries.axisY.viewportMinimum &&
              entry.dataPoint.y <= entry.dataSeries.axisY.viewportMaximum
            )
              entries.push(entry);
          }
    ...
    if (this.enabled) {
      ...
      if (toolTipInnerHtml !== null) {
                ...
                : entries[0].dataPoint.color
                  ...
                  : entries[0].dataSeries.color
                    ...
                    : entries[0].dataSeries._colorSet[
                        entries[0].index %
                          entries[0].dataSeries._colorSet.length
                      ];
        ...
        if (
          ...
          toolTipLeft =
            (entries[0].dataSeries.axisX.conversionParameters.reference +
              entries[0].dataSeries.axisX.conversionParameters.pixelPerUnit *
                (entries[0].dataPoint.x -
                  entries[0].dataSeries.axisX.conversionParameters.minimum) -
        ...
        if (
          ...
          toolTipBottom =
            (entries[0].dataSeries.axisY.conversionParameters.reference +
              entries[0].dataSeries.axisY.conversionParameters.pixelPerUnit *
                (entries[0].dataPoint.y -
                  entries[0].dataSeries.axisY.viewportMinimum) +
              0.5) <<
            0;
        } else if (
          entries[0].dataSeries.type === "bar" ||
...
ToolTip.prototype.highlightObjects = function (entries) {
  ...
  overlaidCanvasCtx.rect(
    plotArea.x1,
    plotArea.y1,
    plotArea.x2 - plotArea.x1,
    plotArea.y2 - plotArea.y1,
  );
    ...
    if (
      ...
      if (
        ...
        drawSegment(
          ...
          0.3,
          eventObject.percentInnerRadius,
        );
