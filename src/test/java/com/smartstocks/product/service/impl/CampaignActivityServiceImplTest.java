...
import com.smartstocks.product.service.renderer.HandlebarsRenderer;
...
public class CampaignActivityServiceImplTest {
    ...
    @Spy
    private ITemplateRenderer templateRenderer = new HandlebarsRenderer();
    ...
    void testTrigger_FetchesDataFromApi() {
        ...
        // We use GMAIL provider and let it fail on auth/send, since we only want to
        // test the renderer setup
        campaign.setEmailProviderType(EmailProviderType.GMAIL);

        Template template = new Template();
        template.setId(1L);
        template.setSubject("Test Subject");
        template.setHtmlBody("Articles: {{#each data}}{{this.title}}{{/each}}");
        ...
        when(templateRendererFactory.get(RendererType.HANDLEBARS)).thenReturn(templateRenderer);
        ...
        try {
            ...
            // Ignore exception if it fails downstream (like email provider), we just care
            // about renderer variables
        }
        ...
        // Let's verify the loop rendering by calling render manually with the captured
        // variables
        ...
        assertTrue(outputHtml.contains("Mock Title"),
                "The for-loop should have rendered the article title.");
    }
}
